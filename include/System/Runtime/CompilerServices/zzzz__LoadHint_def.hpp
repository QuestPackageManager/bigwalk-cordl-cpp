#pragma once
// IWYU pragma private; include "System/Runtime/CompilerServices/LoadHint.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(LoadHint)
// Forward declare root types
namespace System::Runtime::CompilerServices {
struct LoadHint;
}
// Write type traits
MARK_VAL_T(::System::Runtime::CompilerServices::LoadHint);
DEFINE_IL2CPP_CLASS(::System::Runtime::CompilerServices::LoadHint, "System.Runtime.CompilerServices", "LoadHint");
// Dependencies 
namespace System::Runtime::CompilerServices {
// Is value type: true
// CS Name: System.Runtime.CompilerServices.LoadHint
struct CORDL_TYPE LoadHint {
public:
// Declarations
using __CORDL_BACKING_ENUM_TYPE = int32_t;

/// @brief Nested struct __LoadHint_Unwrapped
enum struct __LoadHint_Unwrapped : int32_t {
__E_Default = static_cast<int32_t>(0x0),
__E_Always = static_cast<int32_t>(0x1),
__E_Sometimes = static_cast<int32_t>(0x2),
};

/// @brief Conversion into unwrapped enum value
constexpr operator __LoadHint_Unwrapped () const noexcept {
return static_cast<__LoadHint_Unwrapped>(this->value__);
}

/// @brief Conversion into unwrapped enum value
constexpr explicit operator int32_t () const noexcept {
return static_cast<int32_t>(this->value__);
}

// Ctor Parameters []
// @brief default ctor
constexpr LoadHint() ;

// Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
constexpr LoadHint(int32_t  value__) noexcept;

/// @brief Field Always value: I32(1)
static ::System::Runtime::CompilerServices::LoadHint const Always;

/// @brief Field Default value: I32(0)
static ::System::Runtime::CompilerServices::LoadHint const Default;

/// @brief Field Sometimes value: I32(2)
static ::System::Runtime::CompilerServices::LoadHint const Sometimes;

/// @brief IL2CPP Metadata Type Index
static constexpr uint32_t  __IL2CPP_TYPE_DEFINITION_INDEX{1193};

/// @brief The size of the true value type
static constexpr auto  __IL2CPP_VALUE_TYPE_SIZE{0x4};

/// @brief Field value__, offset: 0x4, size: 0x4, def value: None
 int32_t  value__;

static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::System::Runtime::CompilerServices::LoadHint, value__) == 0x4, "Offset mismatch!");

static_assert(sizeof(::System::Runtime::CompilerServices::LoadHint) == 0x4, "Size mismatch!");

} // namespace end def System::Runtime::CompilerServices
