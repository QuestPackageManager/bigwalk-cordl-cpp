#pragma once
// IWYU pragma private; include "System/MidpointRounding.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(MidpointRounding)
// Forward declare root types
namespace System {
struct MidpointRounding;
}
// Write type traits
MARK_VAL_T(::System::MidpointRounding);
DEFINE_IL2CPP_CLASS(::System::MidpointRounding, "System", "MidpointRounding");
// Dependencies 
namespace System {
// Is value type: true
// CS Name: System.MidpointRounding
struct CORDL_TYPE MidpointRounding {
public:
// Declarations
using __CORDL_BACKING_ENUM_TYPE = int32_t;

/// @brief Nested struct __MidpointRounding_Unwrapped
enum struct __MidpointRounding_Unwrapped : int32_t {
__E_ToEven = static_cast<int32_t>(0x0),
__E_AwayFromZero = static_cast<int32_t>(0x1),
};

/// @brief Conversion into unwrapped enum value
constexpr operator __MidpointRounding_Unwrapped () const noexcept {
return static_cast<__MidpointRounding_Unwrapped>(this->value__);
}

/// @brief Conversion into unwrapped enum value
constexpr explicit operator int32_t () const noexcept {
return static_cast<int32_t>(this->value__);
}

// Ctor Parameters []
// @brief default ctor
constexpr MidpointRounding() ;

// Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
constexpr MidpointRounding(int32_t  value__) noexcept;

/// @brief Field AwayFromZero value: I32(1)
static ::System::MidpointRounding const AwayFromZero;

/// @brief Field ToEven value: I32(0)
static ::System::MidpointRounding const ToEven;

/// @brief IL2CPP Metadata Type Index
static constexpr uint32_t  __IL2CPP_TYPE_DEFINITION_INDEX{274};

/// @brief The size of the true value type
static constexpr auto  __IL2CPP_VALUE_TYPE_SIZE{0x4};

/// @brief Field value__, offset: 0x4, size: 0x4, def value: None
 int32_t  value__;

static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::System::MidpointRounding, value__) == 0x4, "Offset mismatch!");

static_assert(sizeof(::System::MidpointRounding) == 0x4, "Size mismatch!");

} // namespace end def System
