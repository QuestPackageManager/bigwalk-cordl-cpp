#pragma once
// IWYU pragma private; include "WaveHarmonic/Crest/TimeProvider.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "WaveHarmonic/Crest/Internal/zzzz__ManagedBehaviour_1_def.hpp"
#include <cmath>
CORDL_MODULE_EXPORT(TimeProvider)
namespace WaveHarmonic::Crest {
class ITimeProvider;
}
namespace WaveHarmonic::Crest {
class WaterRenderer;
}
// Forward declare root types
namespace WaveHarmonic::Crest {
class TimeProvider;
}
// Write type traits
MARK_REF_T(::WaveHarmonic::Crest::TimeProvider*);
DEFINE_IL2CPP_CLASS(::WaveHarmonic::Crest::TimeProvider*, "WaveHarmonic.Crest", "TimeProvider");
// Dependencies WaveHarmonic.Crest.Internal.ManagedBehaviour`1<T>
namespace WaveHarmonic::Crest {
// Is value type: false
// CS Name: WaveHarmonic.Crest.TimeProvider
class CORDL_TYPE TimeProvider : public ::WaveHarmonic::Crest::Internal::ManagedBehaviour_1<::UnityW<::WaveHarmonic::Crest::WaterRenderer>> {
public:
// Declarations
 __declspec(property(get=get_Delta)) float_t  Delta;

 __declspec(property(get=get_Time)) float_t  Time;

/// @brief Convert operator to "::WaveHarmonic::Crest::ITimeProvider"
constexpr operator  ::WaveHarmonic::Crest::ITimeProvider*() noexcept;

static inline ::WaveHarmonic::Crest::TimeProvider* New_ctor() ;

/// @brief Method .ctor, addr 0x18256fed0, size 0x10, virtual false, abstract: false, final false
inline void _ctor() ;

/// @brief Method get_Delta, addr 0xffffffffffffffff, size 0xffffffffffffffff, virtual true, abstract: false, final false
inline float_t get_Delta() ;

/// @brief Method get_Time, addr 0xffffffffffffffff, size 0xffffffffffffffff, virtual true, abstract: false, final false
inline float_t get_Time() ;

/// @brief Convert to "::WaveHarmonic::Crest::ITimeProvider"
constexpr ::WaveHarmonic::Crest::ITimeProvider* i___WaveHarmonic__Crest__ITimeProvider() noexcept;

protected:
// Ctor Parameters []
// @brief default ctor
constexpr TimeProvider() ;
public:

// Ctor Parameters [CppParam { name: "", ty: "TimeProvider", modifiers: "&&", def_value: None }]
// @brief delete move ctor to prevent accidental deref moves
TimeProvider(TimeProvider && ) = delete;

// Ctor Parameters [CppParam { name: "", ty: "TimeProvider", modifiers: "const&", def_value: None }]
// @brief delete copy ctor to prevent accidental deref copies
TimeProvider(TimeProvider const& ) = delete;

/// @brief IL2CPP Metadata Type Index
static constexpr uint32_t  __IL2CPP_TYPE_DEFINITION_INDEX{16701};

static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(sizeof(::WaveHarmonic::Crest::TimeProvider) == 0x50, "Size mismatch!");

} // namespace end def WaveHarmonic::Crest
