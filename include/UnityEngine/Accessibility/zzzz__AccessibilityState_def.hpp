#pragma once
// IWYU pragma private; include "UnityEngine/Accessibility/AccessibilityState.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(AccessibilityState)
// Forward declare root types
namespace UnityEngine::Accessibility {
struct AccessibilityState;
}
// Write type traits
MARK_VAL_T(::UnityEngine::Accessibility::AccessibilityState);
DEFINE_IL2CPP_CLASS(::UnityEngine::Accessibility::AccessibilityState, "UnityEngine.Accessibility", "AccessibilityState");
// Dependencies 
namespace UnityEngine::Accessibility {
// Is value type: true
// CS Name: UnityEngine.Accessibility.AccessibilityState
struct CORDL_TYPE AccessibilityState {
public:
// Declarations
using __CORDL_BACKING_ENUM_TYPE = uint8_t;

/// @brief Nested struct __AccessibilityState_Unwrapped
enum struct __AccessibilityState_Unwrapped : uint8_t {
__E_None = static_cast<uint8_t>(0x0u),
__E_Disabled = static_cast<uint8_t>(0x1u),
__E_Selected = static_cast<uint8_t>(0x2u),
__E_Expanded = static_cast<uint8_t>(0x4u),
};

/// @brief Conversion into unwrapped enum value
constexpr operator __AccessibilityState_Unwrapped () const noexcept {
return static_cast<__AccessibilityState_Unwrapped>(this->value__);
}

/// @brief Conversion into unwrapped enum value
constexpr explicit operator uint8_t () const noexcept {
return static_cast<uint8_t>(this->value__);
}

// Ctor Parameters []
// @brief default ctor
constexpr AccessibilityState() ;

// Ctor Parameters [CppParam { name: "value__", ty: "uint8_t", modifiers: "", def_value: None }]
constexpr AccessibilityState(uint8_t  value__) noexcept;

/// @brief Field Disabled value: U8(1)
static ::UnityEngine::Accessibility::AccessibilityState const Disabled;

/// @brief Field Expanded value: U8(4)
static ::UnityEngine::Accessibility::AccessibilityState const Expanded;

/// @brief Field None value: U8(0)
static ::UnityEngine::Accessibility::AccessibilityState const None;

/// @brief Field Selected value: U8(2)
static ::UnityEngine::Accessibility::AccessibilityState const Selected;

/// @brief IL2CPP Metadata Type Index
static constexpr uint32_t  __IL2CPP_TYPE_DEFINITION_INDEX{21223};

/// @brief The size of the true value type
static constexpr auto  __IL2CPP_VALUE_TYPE_SIZE{0x1};

/// @brief Field value__, offset: 0x1, size: 0x1, def value: None
 uint8_t  value__;

static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::UnityEngine::Accessibility::AccessibilityState, value__) == 0x1, "Offset mismatch!");

static_assert(sizeof(::UnityEngine::Accessibility::AccessibilityState) == 0x1, "Size mismatch!");

} // namespace end def UnityEngine::Accessibility
